    ...
    try:
        ...
        if 'role' not in columns:
            ...
            with engine.connect() as conn:
                conn.execute("ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'teacher'")
                conn.commit()
        ...
        if 'status' in achievement_columns:
            ...
            achievements = db.query(Achievement).filter(
                (Achievement.status == None) | (Achievement.status == '')
            ).all()
            ...
            if achievements:
                for ach in achievements:
                    ach.status = 'approved'
                db.commit()
                print(f"✅ Установлен статус 'approved' для {len(achievements)} достижений")
